<?php namespace BugTrap;

use Exception;

use BugTrap\Helpers\Logger;
...
class BugTrap
{
    ...
    {
        $this->config['except'] = config('bugtrap.except', []);
        $this->config['count'] = config('bugtrap.lines_count', 12);
        $this->config['environments'] = config('bugtrap.environments', []);
        $this->config['sleep'] = config('bugtrap.sleep', 0);
    ...
    {
        return 'bugtrap.' . str_slug($data['host'] . '_' . $data['method'] . '_' . $data['exception'] . '_' . $data['line'] . '_' . $data['file'] . '_' . $data['class']);
    }
